...
Training the models for 8 hours overnight leads to performance which is slightly better, but still far short of performance achieved for example [7 years ago by Andrej Karpathy](http://karpathy.github.io/2015/05/21/rnn-effectiveness/). Below are training curves from the 8 hour training sessions for both models (the periodic nature of the training curves are presumably due to certain parts of the training data which the model finds surprising every time it encounters them):

![](./Results/Protected/overnight_Shakespeare_RNN_training_curve.png)

![](./Results/Protected/overnight_Shakespeare_LSTM_training_curve.png)

The console output from the overnight training sessions is included for both the [RNN](./Results/Protected/RNN_training_output.txt) and the [LSTM](./Results/Protected/LSTM_training_output.txt). Below are some examples of the text generated by the RNN during the later stages of training:

```
! chang llis.
 mast they lost, that shahid coktare me bede'ng no [ipe terch a coand debegy you
 gind-haopge will mo.
 and for thes blay de'tryors!
 and wine to sinfer anchund wost,
 be sfruth
 nache!
 my herk kaldis thene-; s'in love suly purthem surny harld,
 oggeeg,
 sutkun, came'in- un thy this. shill my the afthe, naid stood cay ploughs, go dit sir, iht? youl froo, a that tis he leved,
 meln.
 to my i celly not my aster
 to haar; hare moye gid's roy.
 me enstind grerion,
 fer, gosth the you,
```
...
'o. the dim thetrsean.
 a isst froess
 detilo
 bugd, share the meef well,
 wims of falnou, ann!e say not wish wally, more that siad, wing is proon, minm, we him the fack hesternit. he filling?r wis, strerge
 doflok.
 whaveno and
 he nots-slane wirciic couln, youl ofy is the vawe, i bely ectow maldented nate. o, i where
 thun froull s pallinoued for'was hucver;
 wave traed no frad i shrard
 troullss davay, poth byess,
 ut the dike af's filk, of piik,
...
The saved models are also included in this repository, for both the [RNN](./Results/shakespeare_rnn.pkl) and the [LSTM](./Results/shakespeare_lstm.pkl).
